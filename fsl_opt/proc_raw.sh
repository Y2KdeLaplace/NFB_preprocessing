...
OUT_DIR="/mnt/c/rtfmri/nii_files/"
mkdir -p "$OUT_DIR"
cd "$OUT_DIR" || exit
...
hd-bet --input "$RAW_T1" --output raw_T1_brain.nii.gz --save_bet_mask
mv raw_T1_brain_bet.nii.gz tmp_raw_T1_brain_mask.nii.gz
...
if [ "$USE_FMAP" -eq 1 ]; then
    ...
    hd-bet --input "$RAW_FMAP_MAG" --output tmp_fmap_mag_brain.nii.gz
    fsl_prepare_fieldmap SIEMENS "$RAW_FMAP_PH" tmp_fmap_mag_brain.nii.gz tmp_fmap_rads.nii $DELTA_TE
fi
...
if [ "$USE_FMAP" -eq 0 ]; then
    ...
    epi_reg --epi=MC_Templ.nii \
            ...
            --fmapmagbrain=tmp_fmap_mag_brain.nii.gz \
...
if [ "$USE_FMAP" -eq 1 ]; then
    ...
    applywarp --ref=tmp_MC_Templ_1mm.nii --in=raw_T1_brain.nii.gz --warp=T1_to_distorted_EPI_warp.nii --out=T1.nii
else
    ...
    flirt -in raw_T1_brain.nii.gz -ref MC_Templ.nii -applyxfm -init raw_T1_brain_to_EPI.mat -applyisoxfm 1 -out T1.nii
fi
